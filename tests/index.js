  ...
  const runEmit = opts => run(opts)
    .then((compilation) => {
      ...
      if (opts.expectedAssetContent) {
        for (const key in opts.expectedAssetContent) { // eslint-disable-line no-restricted-syntax
          expect(compilation.assets[key]).to.exist; // eslint-disable-line no-unused-expressions
          if (compilation.assets[key]) {
            const expectedContent = opts.expectedAssetContent[key];
            const compiledContent = compilation.assets[key].source().toString();
